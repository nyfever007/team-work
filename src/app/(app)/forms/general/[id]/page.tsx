...
import { cancelGeneralRequest, decideGeneralRequest } from "@/lib/general/actions";
import { generalById } from "@/lib/general/queries";
import { RETENTION_LABEL, amountLine, isRetention } from "@/lib/general/types";
...
export default async function GeneralDetailPage({ params }: PageProps<"/forms/general/[id]">) {
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <Button asChild>
          <a href={`/print/general/${req.id}`} target="_blank" rel="noopener">
            <PrinterIcon />
            인쇄
          </a>
        </Button>
      ...
      {req.status === "submitted" && (
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-brand/25 bg-brand-soft/70 px-4 py-3 text-sm">
          ...
          {access.canApprove(req) && <ApprovalActions decide={decideGeneralRequest.bind(null, req.id)} title={`${req.memberName} 일반 품의`} />}
        </div>
      )}
      {req.status === "rejected" && (
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 text-sm sm:grid-cols-2">
          <div className="sm:col-span-2">
            <Field k="목적"><span className="whitespace-pre-wrap">{req.purpose}</span></Field>
          </div>
          <Field k="거래처">{req.vendor || "—"}</Field>
          <Field k="기간">{req.period || "—"}</Field>
          <Field k="시기 (지급일자)">{req.timing || "—"}</Field>
          <Field k="금액"><b>{amountLine(req.amount, req.currency, req.vat) || "—"}</b></Field>
          <Field k="지급계좌">{req.account || "—"}</Field>
          <Field k="보존기간">{isRetention(req.retention) ? RETENTION_LABEL[req.retention] : "—"}</Field>
          {req.extra && (
            <div className="sm:col-span-2">
              <Field k="추가 내용"><span className="whitespace-pre-wrap">{req.extra}</span></Field>
            </div>
          )}
          <div className="sm:col-span-2">
      ...
      {(req.status === "submitted" || req.status === "approved") && access.canCancel(req) && (
        <div className="flex items-center justify-between gap-3 rounded-lg border px-4 py-3 text-sm">
          ...
          <CancelRequestButton cancel={cancelGeneralRequest.bind(null, req.id)} docNo={req.docNo} approved={req.status === "approved"} approvedText="승인된 품의를 취소합니다. 취소된 문서는 목록에 남습니다." />
        </div>
      )}
    </div>
  );
