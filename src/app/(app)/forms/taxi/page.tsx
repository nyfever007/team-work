...
import { taxiFor } from "@/lib/taxi/queries";
...
const md = (k: string) => k.slice(5).replace("-", "/");
...
export default async function TaxiListPage() {
  ...
  const rows = taxiFor(visible.map((m) => m.id));
  const year = todayKey().slice(0, 4);
  const myApproved = rows.filter((r) => r.status === "approved" && r.memberId === access.me?.id && r.writtenAt.startsWith(year)).reduce((n, r) => n + r.amount, 0);
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className="grid size-10 place-items-center rounded-xl bg-amber-50 text-amber-700">
            <FormIcon kind="taxi" className="size-5" />
          </span>
          <div>
            <h2 className="text-lg font-bold">택시비 지급 품의서</h2>
            <p className="text-sm text-muted-foreground">
              {access.isAdmin ? "전체" : access.me?.isLeader ? "우리 팀" : "내"} 품의서 {rows.length}건{access.me && ` · 올해 승인된 내 택시비 ${myApproved.toLocaleString("ko-KR")}원`}
            </p>
          </div>
        </div>
        <Button asChild>
          <Link href="/forms/taxi/new">
            <PlusIcon />
            품의서 작성
          </Link>
        </Button>
        ...
        <table className="w-full min-w-[720px] text-sm">
          <thead className="border-b bg-muted/40 text-xs text-muted-foreground">
            <tr>
              ...
              <th className="px-3 py-2.5 text-left font-medium">이용 기간</th>
            ...
            {rows.map((r) => (
              <tr key={r.id} className={cn("relative border-b last:border-b-0 hover:bg-accent/40", (r.status === "cancelled" || r.status === "rejected") && "text-muted-foreground")}>
                <td className="px-4 py-2.5 font-mono text-xs">
                  <Link href={`/forms/taxi/${r.id}`} className="hover:underline after:absolute after:inset-0">{r.docNo}</Link>
                </td>
                <td className="px-3 py-2.5">
                  <div className="font-medium">{r.memberName}</div>
                  <div className="text-xs text-muted-foreground">{r.teamName}</div>
                </td>
                <td className="px-3 py-2.5 whitespace-nowrap tabular-nums">{r.useStart === r.useEnd ? md(r.useStart) : `${md(r.useStart)} ~ ${md(r.useEnd)}`}</td>
