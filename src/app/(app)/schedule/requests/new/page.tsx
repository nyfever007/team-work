...
export default async function NewRequestPage({ searchParams }: PageProps<"/schedule/requests/new">) {
  ...
  const memberInfo = members.map((m) => {
    ...
    return { id: m.id, name: m.name, team: m.team, position: m.rank || m.position, totalDays: b.annual.accrued, annualTotal: b.annual.total, usedDays: b.annual.used, sickUsed: b.sick.used, sickAllowance: b.sick.allowance, period: `${b.period.start} ~ ${b.period.end}`, yearIndex: b.period.yearIndex };
  });
  const defaultMemberId = access.me?.id ?? members[0].id;
  const delegates = allMembers().map((m) => ({ id: m.id, name: m.name, team: m.team, phone: m.phone }));

  return (
    <div className="grid gap-4">
      <div>
        <h2 className="text-lg font-semibold">휴가 품의서 작성</h2>
        <p className="text-sm text-muted-foreground">소속·직위·성명·신청일수·잔여일수는 자동으로 채워집니다. 저장하면 달력에 반영되고 바로 인쇄할 수 있습니다.</p>
      </div>
      <Card>
